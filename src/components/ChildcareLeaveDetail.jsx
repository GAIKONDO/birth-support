...
const ChildcareLeaveDetail = () => {
              ...
              nodeRects.forEach((rect) => {
                ...
                if (node) {
                  ...
                  if (foreignObject) {
                    ...
                    const isDescNode = nodeText.includes('制度の設計と監督を行う国の機関') ||
                                      nodeText.includes('雇用保険の給付を行う機関') ||
                                      nodeText.includes('育児休業を取得し給付金を受給する当事者') ||
                                      nodeText.includes('雇用主。申請手続きを代行');
  ...
  const architectureDiagram = `
    graph TB
      ...
      subgraph areaA[" "]
        A["👤 あなた<br/>（被保険者）"]
        A_DESC["育児休業を取得し給付金を受給する当事者"]
      end
      
      subgraph areaB[" "]
        B["🏢 ハローワーク"]
        B_DESC["雇用保険の給付を行う機関。育児休業給付金を支給"]
      end
  ...
  const componentDiagram = `
    graph LR
      ...
      C --> C1[休業開始から180日目まで<br/>休業開始時賃金日額の67%]
      C1 --> C2[181日目以降<br/>休業開始時賃金日額の50%]
      
      D --> D1[原則として子が1歳に達するまで]
      D1 --> D2[延長可能<br/>保育所に入所できない等の事情がある場合<br/>最長で子が2歳に達するまで]
        ...
        <div className="detail-section mermaid-section">
          ...
          <div className="detail-card" ref={fullscreenRef}>
            ...
            {viewMode === 'correlation' && (
              <>
                <p className="section-description">
                  育児休業給付金制度に関わる主要な組織とその関係性を示します。<strong className="highlight-text">紫色の枠で囲まれた「あなた」が当事者（被保険者）の位置</strong>です。図中の数字は手続きの流れを示しています。
                </p>
                <div className="mermaid-container">
                  <MermaidChart chart={architectureDiagram} id="childcare-leave-correlation" zoomLevel={zoomLevel} />
                </div>
                <div className="diagram-legend">
                  <h4 className="legend-title">図の見方</h4>
                  <ul className="legend-list">
                    <li><strong className="legend-you">👤 あなた（被保険者）</strong>：育児休業を取得し、育児休業給付金を受給する当事者です。雇用保険に加入している方が対象です。</li>
                    <li><strong>🏢 ハローワーク</strong>：雇用保険の給付を行う機関です。育児休業給付金の支給を行います。</li>
                    <li><strong>💼 勤務先</strong>：雇用主です。育児休業給付金の申請手続きを代行します。</li>
            ...
            {viewMode === 'components' && (
              <>
                <p className="section-description">
                  育児休業給付金制度の構成要素を樹形図で示します。
                </p>
                <div className="mermaid-container">
                  <MermaidChart chart={componentDiagram} id="childcare-leave-component" zoomLevel={zoomLevel} />
                </div>
              </>
            )}
              ...
              <>
                <p className="section-description">
                  ...
                  <span style={{ fontSize: '13px', color: '#dc2626', fontStyle: 'italic' }}>
                    ※育児休業給付金の申請は、育児休業開始日から4か月以内に行う必要があります。また、支給期間を延長する場合（1歳6か月や2歳まで）は、所定の期限内に延長申請を行い、必要な証明書類を提出する必要があります。職場復帰日が変更になった場合は、速やかに報告してください。給付金は約2か月に1回の割合で支給されます。
                  </span>
                </p>
                <div className="mermaid-container">
                  <MermaidChart chart={applicationFlowSequence} id="childcare-leave-flow" zoomLevel={zoomLevel} />
                </div>
                <div className="flow-explanation" style={{ marginTop: '24px', padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px', fontSize: '14px', lineHeight: '1.6' }}>
                  ...
                  <p style={{ margin: 0, marginBottom: '24px', color: '#6b7280' }}>
                    ハローワーク（正式名称：公共職業安定所）は、日本政府（厚生労働省）が運営する公的な就業支援機関です。主に「仕事を探している人」と「人材を採用したい企業」をつなぐ役割を担っています。育児休業給付金の申請においては、雇用保険の給付を行う機関として、申請書類の審査や給付金の支給を行います。
                  </p>
                  ...
                  <ol style={{ margin: 0, paddingLeft: '20px', color: '#6b7280' }}>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>育児休業の取得申出</strong><br />
                      被保険者が勤務先に育児休業を取得する意思を伝えます。育児休業の開始予定日や終了予定日を明確にすることが重要です。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>必要書類の提出</strong><br />
                      被保険者が勤務先に必要書類を提出します。母子手帳の写しや給付金を受け取る口座情報などが必要です。これらの書類は申請手続きに使用されます。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>申請書類の作成</strong><br />
                      勤務先が申請に必要な書類を作成します。受給資格確認票や育児休業給付金支給申請書などを作成し、被保険者の情報を記入します。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                    ...
                    <li style={{ marginBottom: '12px' }}>
                      ...
                      ハローワークが被保険者の指定口座に給付金を振り込みます。給付金は約2か月ごとに支給され、休業開始から180日目までは休業開始時賃金日額の67%、181日目以降は50%が支給されます。
                    </li>
          ...
          <div className="detail-card">
            <table className="overview-table">
              <tbody>
                <tr>
                  <th>概要</th>
                  <td>育児休業中に受け取れる給付金です。雇用保険から支給されます。</td>
                </tr>
                <tr>
                  <th>支給金額</th>
                  <td>
                    <span className="amount-highlight-inline">休業開始時賃金の67%（6ヶ月後は50%）</span>
                  </td>
                </tr>
                <tr>
                  <th>対象者</th>
                ...
                <tr>
                  ...
                  <td>
                    <a 
                      href="https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/koyou_roudou/koyou/kyufukin/index.html" 
                      target="_blank" 
...
export default ChildcareLeaveDetail;
