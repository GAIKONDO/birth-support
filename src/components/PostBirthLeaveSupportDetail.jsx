...
const PostBirthLeaveSupportDetail = () => {
  ...
  const architectureDiagram = `
    graph TB
      ...
      subgraph areaA[" "]
        A["👤 あなた<br/>（両親）"]
        A_DESC["出生後休業支援給付金を受給する当事者"]
      end
      
      subgraph areaB[" "]
        B["🏢 ハローワーク"]
        B_DESC["雇用保険の給付を行う機関。出生後休業支援給付金を支給"]
      end
      
      D -->|"1. 制度設計・監督<br/>（事務処理）"| B
      E -->|"2. 雇用保険の加入手続き<br/>（事務処理）"| B
      A -->|"3. 雇用保険に加入<br/>（加入）"| B
      A -->|"4. 両親がともに14日以上の<br/>育児休業を取得<br/>（申請）"| E
      E -->|"5. 給付金の申請を代行<br/>（申請代行）"| B
      B -->|"6. 出生後休業支援給付金を支給<br/>（支給）"| A
  ...
  const componentDiagram = `
    graph LR
      A[出生後休業支援給付金制度] --> B[対象者]
      A --> C[支給金額]
      A --> D[支給期間]
      A --> G[申請期限]
      
      B --> B1[雇用保険の被保険者]
      B --> B1_2[両親がともに14日以上の<br/>育児休業を取得する場合]
        ...
        <div className="detail-section mermaid-section">
          ...
          <div className="detail-card" ref={fullscreenRef}>
            ...
            {viewMode === 'correlation' && (
              <>
                <p className="section-description">
                  出生後休業支援給付金制度に関わる主要な組織とその関係性を示します。<strong className="highlight-text">紫色の枠で囲まれた「あなた」が当事者（両親）の位置</strong>です。図中の数字は手続きの流れを示しています。
                </p>
                <div className="mermaid-container">
                  <MermaidChart chart={architectureDiagram} id="post-birth-leave-support-correlation" zoomLevel={zoomLevel} />
                </div>
                <div className="diagram-legend">
                  <h4 className="legend-title">図の見方</h4>
                  <ul className="legend-list">
                    <li><strong className="legend-you">👤 あなた（両親）</strong>：出生後休業支援給付金を受給する当事者です。両親がともに14日以上の育児休業を取得する場合が対象です。</li>
                    <li><strong>🏢 ハローワーク</strong>：雇用保険の給付を行う機関です。出生後休業支援給付金の支給を行います。</li>
                    <li><strong>💼 勤務先</strong>：雇用主です。出生後休業支援給付金の申請手続きを代行します。</li>
            ...
            {viewMode === 'components' && (
              <>
                <p className="section-description">
                  出生後休業支援給付金制度の構成要素を樹形図で示します。
                </p>
                <div className="mermaid-container">
                  <MermaidChart chart={componentDiagram} id="post-birth-leave-support-component" zoomLevel={zoomLevel} />
                </div>
              </>
            )}
            ...
            {viewMode === 'flow' && (
              <>
                <p className="section-description">
                  出生後休業支援給付金の申請フローを示します。
                  <br />
                  <span style={{ fontSize: '13px', color: '#dc2626', fontStyle: 'italic' }}>
                    ※この制度は令和7年4月1日から創設されます。両親がともに14日以上の育児休業を取得する場合、最大28日間、手取り収入の10割相当が支給されます。詳細はこども家庭庁の公式サイトで最新情報を確認してください。
                  </span>
                </p>
                <div className="mermaid-container">
                  <MermaidChart chart={applicationFlowSequence} id="post-birth-leave-support-flow" zoomLevel={zoomLevel} />
                </div>
                <div className="flow-explanation" style={{ marginTop: '24px', padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px', fontSize: '14px', lineHeight: '1.6' }}>
                  ...
                  <p style={{ margin: 0, marginBottom: '24px', color: '#6b7280' }}>
                    ハローワーク（正式名称：公共職業安定所）は、日本政府（厚生労働省）が運営する公的な就業支援機関です。出生後休業支援給付金の申請においては、雇用保険の給付を行う機関として、申請書類の審査や給付金の支給を行います。
                  </p>
                  ...
                  <ol style={{ margin: 0, paddingLeft: '20px', color: '#6b7280' }}>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>両親がともに14日以上の育児休業の取得申出</strong><br />
                      両親がそれぞれ勤務先に育児休業を取得する意思を伝えます。両親がともに14日以上の育児休業を取得する必要があります。育児休業の開始予定日や終了予定日を明確にすることが重要です。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>必要書類の提出</strong><br />
                      両親がそれぞれ勤務先に必要書類を提出します。母子手帳の写しや給付金を受け取る口座情報などが必要です。これらの書類は申請手続きに使用されます。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>申請書類の作成</strong><br />
                      勤務先が申請に必要な書類を作成します。受給資格確認票や出生後休業支援給付金支給申請書などを作成し、両親の情報を記入します。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                      <strong style={{ color: '#374151' }}>申請書類の提出</strong><br />
                      勤務先が作成した申請書類を、事業所所在地を管轄するハローワークに提出します。申請は所定の期限内に行う必要があります。
                    </li>
                    <li style={{ marginBottom: '12px' }}>
                    ...
                    <li style={{ marginBottom: '12px' }}>
                      ...
                      ハローワークが両親の指定口座に給付金を振り込みます。最大28日間、手取り収入の10割相当が支給されます。
                    </li>
                    <li style={{ marginBottom: '0' }}>
                      <strong style={{ color: '#374151' }}>支給実績の報告</strong><br />
                      ハローワークがこども家庭庁に支給実績を報告します。これにより、制度の運用状況が把握され、適切な監督が行われます。
                    </li>
                  </ol>
                </div>
              </>
          ...
          <div className="detail-card">
            <table className="overview-table">
              <tbody>
                <tr>
                  <th>概要</th>
                  <td>両親がともに14日以上の育児休業を取得する場合、最大28日間、休業開始前の給与の13％が支給される制度です。</td>
                </tr>
                <tr>
                  <th>支給金額</th>
                  <td>
                    <span className="amount-highlight-inline">休業開始前の給与の13％（最大28日間）</span>
                  </td>
                </tr>
                <tr>
                  <th>対象者</th>
                ...
                <tr>
                  ...
                  <td>
                    <a 
                      href="https://www.cfa.go.jp/resources/strategy/tomoni-oen" 
                      target="_blank" 
...
export default PostBirthLeaveSupportDetail;
